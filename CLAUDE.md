...
- `internal/config` — loads `.env` + process env into a `Config` struct. No validation at load time; each consumer of a missing value fails with its own clear error (e.g. consumer fails a specific job, not startup). `Timezone` (env `TIMEZONE`, default `Asia/Ho_Chi_Minh`) is the one IANA zone for the whole deployment — schedules' HH:MM (schedules don't carry their own timezone), admin UI timestamp display, and the date/time stamped into every backup filename and storage date-folder all interpret/format through it. Threaded from `cfg.Timezone` down into `backupAndUpload`/`remoteBackupAndUpload` (`cmd/backupdb/consumer.go`) and `runUpload` (`cmd/backupdb/upload.go`) as a `*time.Location`; sent to a remote `agent` as `agentproto.RunRequest.Timezone` (an IANA name string) since the agent runs on a different machine that may be in a different OS timezone and has no config of its own to read this from — `runBackupJob` (`cmd/backupdb/agent.go`) loads it per-request, falling back to UTC if missing/invalid.
- `internal/registry` — SQLite-backed source of truth: `databases` (what to back up and where, plus `agent_id` — 0 runs locally, non-zero routes through a `remote_agents` row; `name` alone is **not** unique — the same name may legitimately repeat across different agents, or across drivers on the same agent, e.g. a mysql "shop" and a mongo "shop" — the unique key is the triple `(name, agent_id, driver)`, `idx_databases_name_agent_driver`, because that's exactly what a `queue.Job` carries and what `Registry.GetByNameAgentDriver` uses in the consumer to resolve a finished job back to its row without risking a name collision picking the wrong one; `Registry.GetByName` still exists for the `upload` CLI's manual dbname lookup, where ambiguity is acceptable since the command already supports an explicit storage-target-id override), `schedules` (per-database HH:MM backup times, any number per database), `shared_schedules`/`shared_schedule_databases`/`shared_schedule_times` (one group of databases, any number of HH:MM triggers, for backing up several databases on the same schedule without duplicating the time on each one), `storage_targets` (configured upload destinations), `notify_channels`/`database_notify_channels` (configured notification destinations, any number assigned per database), `backup_runs` (append-only history log of every finished job, success or error, shown on the admin "Nhật ký" page in place of reading container stdout — no FK to `databases` since a run must stay visible after its database is renamed/deleted), `backup_files` (one row per successfully uploaded file — dbname, storage_target_id, filename, an opaque `remote_ref` (gdrive file ID or s3 object key), size — backing the admin UI's per-database "File" list/download page; same no-FK-on-`database_id` reasoning as `backup_runs`), `remote_agents` (one row per registered `agent` endpoint — label, endpoint URL, Bearer token, pinned TLS cert fingerprint; see "Remote agents" below). All `*_run_date`-tracked tables use that column to avoid double-firing the same day. Schema lives in `internal/registry/schema.go` and is applied idempotently (`CREATE TABLE IF NOT EXISTS`) on every `Open` — **no built-in migration system**, so a schema change requires either a hand-written one-time migration in `registry.Open` (see `migrateSharedScheduleTimes`/`migrateNotifyChannelEvents`/`migrateAgentIDColumn` for the pattern: check column presence via `PRAGMA table_info`, then `ALTER TABLE`/backfill) or wiping the `sqlite-data` volume. `migrateDatabasesNameUniqueScope` is the one migration so far that needs more than `ALTER TABLE` — replacing the old single-column `UNIQUE(name)` with the composite `(name, agent_id, driver)` index isn't expressible as an `ALTER TABLE`, so it rebuilds the `databases` table (`PRAGMA foreign_keys=OFF` → create/copy/drop/rename inside a transaction → `PRAGMA foreign_key_check` → `foreign_keys=ON`), preserving every row's `id` so `schedules`/`shared_schedule_databases` (which reference `databases(id)`) need no changes of their own.
...
- `internal/dump` — shells out to `mysqldump` / `pg_dump` / `mongodump` over TCP, streaming stdout through gzip into a tmp file. MySQL password is passed via `MYSQL_PWD` env var, never on the command line. `ParseParams` passes `host` through as entered, no rewriting — a database installed directly on the Docker host (not in a container) needs `host` set to `host.docker.internal` explicitly in the admin UI (mapped via `extra_hosts` in `docker-compose.yml`/`docker-compose.agent.yml`), since neither the consumer's nor the agent's own loopback ever has the target database.
...
- Storage folder/filename layout (`{rootFolder}/{dbname}/{date}/{dbname}_{date}_{stamp}.ext.gz`) is keyed by `dbname` alone, not `(name, agent_id, driver)` — two databases that deliberately share a name (allowed, see `internal/registry` above) still land their backups in the same Drive folder/S3 prefix. Job routing/notify/logging stay correct either way; give databases distinct names if you also want their backups organized into separate folders.
