...
// param string used on the Redis queue and stored in the registry, as-is —
// no host rewriting. A database installed directly on the Docker host (not
// in a container) needs "host" set to host.docker.internal explicitly in
// the admin UI; docker-compose.yml/docker-compose.agent.yml map that via
// extra_hosts.
